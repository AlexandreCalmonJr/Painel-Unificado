...
import 'package:painel_windowns/devices/utils/helpers.dart'; // Presumido
...
// Você deve mover isso para o ModuleManagementService real
extension ModuleManagementServiceAssetHistory on ModuleManagementService {
  ...
  Future<List<Map<String, dynamic>>> fetchAssetHistory(String token, String assetId) async {
    // A implementação real está no module_management_service.dart
...
class _AssetDetailScreenState extends State<AssetDetailScreen> {
  // ✅ CORRIGIDO: Instanciando o serviço corretamente
  ...
  Future<List<Map<String, dynamic>>> _fetchAssetHistory() async {
    ...
    try {
      ...
      // ✅ CORRIGIDO: Chamando o método real do serviço
                    ...
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: _buildSection('Informações Básicas', [
                          _buildDetailRow('Serial', widget.asset.serialNumber,
                              Icons.qr_code,
                              copyable: true),
                          _buildDetailRow('Localização',
                              widget.asset.location ?? 'N/D', Icons.location_on),
                          _buildDetailRow(
                              'Unidade', widget.asset.unit ?? 'N/D', Icons.business),
                          _buildDetailRow(
                              'Setor', widget.asset.sector ?? 'N/D', Icons.domain),
                          _buildDetailRow(
                              'Andar', widget.asset.floor ?? 'N/D', Icons.layers),
                          _buildDetailRow(
                              'Atribuído a',
                          ...
                          _buildDetailRow(
                              'Última Conexão',
                              _formatDateTime(widget.asset.lastSeen),
                              Icons.access_time),
                        ]),
                      ),
                    ),
                                      ...
                                      if (snapshot.hasData &&
                                        ...
                                        return Column(
                                          children: snapshot.data!.map((entry) {
                                            ...
                                            // ✅ CORRIGIDO: Usando formatDateTime global
                    ...
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: _buildSection('Dados Customizados', [
                          _buildDetailRow(
                              'Campo 1',
                              widget.asset.customData['custom_field_1']
                                      ?.toString() ??
                                  'N/A',
                              Icons.info),
                          _buildDetailRow(
                              'Campo 2',
                              widget.asset.customData['custom_field_2']
                                      ?.toString() ??
                                  'N/A',
                              Icons.info),
                        ]),
  ...
  SliverAppBar _buildHeader() {
    ...
    final status = statusInfo[widget.asset.status.toLowerCase()] ?? // ✅ Adicionado toLowerCase()
        {
          'color': Colors.grey,
  ...
  Widget _buildTimelineTile({required String title, required String subtitle}) {
    // ✅ CORRIGIDO: Lógica do _buildTimelineTile
  ...
  Widget _buildDesktopDetails(Desktop desktop) => _buildTypeDetails([
        _buildSection('Identificação', [
          _buildDetailRow('Hostname', desktop.hostname, Icons.dns,
              copyable: true),
          _buildDetailRow(
              'Modelo', desktop.model, Icons.laptop_chromebook),
          _buildDetailRow(
              'Fabricante', desktop.manufacturer, Icons.business_center),
        ]),
        _buildSection('Hardware', [
          ...
          _buildDetailRow(
              'Armazenamento', desktop.storage, Icons.sd_storage),
          _buildDetailRow(
              'Tipo de HD', desktop.storageType, Icons.data_usage),
        ]),
        ...
        _buildSection('Rede', [
          _buildDetailRow(
              'Endereço IP', desktop.ipAddress, Icons.network_check,
              copyable: true),
          _buildDetailRow('MAC Address', desktop.macAddress, Icons.router,
              copyable: true),
        ]),
        _buildSection('Periféricos', [
          _buildDetailRow('Leitor Biométrico',
              desktop.biometricReader ?? 'N/D', Icons.fingerprint),
          _buildDetailRow('Impressora Conectada',
        ...
        _buildSection('Segurança', [
          _buildDetailRow('Antivírus',
              desktop.antivirusStatus ? 'Ativo' : 'Inativo', Icons.security),
          if (desktop.antivirusVersion != null)
            _buildDetailRow(
                'Versão Antivírus', desktop.antivirusVersion!, Icons.verified_user),
        ]),
      ]);

  Widget _buildNotebookDetails(Notebook notebook) => _buildTypeDetails([
        _buildSection('Identificação', [
          _buildDetailRow('Hostname', notebook.hostname, Icons.dns,
              copyable: true),
        ...
        _buildSection('Hardware', [
          ...
          _buildDetailRow(
              'Armazenamento', notebook.storage, Icons.sd_storage),
        ]),
        _buildSection('Bateria', [
          ...
          _buildDetailRow(
              'Saúde', notebook.batteryHealth ?? 'N/D', Icons.health_and_safety),
        ]),
        _buildSection('Rede', [
          _buildDetailRow(
              'Endereço IP', notebook.ipAddress, Icons.network_check,
              copyable: true),
          _buildDetailRow('MAC Address', notebook.macAddress, Icons.router,
              copyable: true),
        ]),
        _buildSection('Segurança', [
          _buildDetailRow('Antivírus',
              notebook.antivirusStatus ? 'Ativo' : 'Inativo', Icons.security),
          _buildDetailRow('Criptografia',
              notebook.isEncrypted ? 'Ativa' : 'Inativa', Icons.lock),
        ]),
      ]);
  ...
  Widget _buildPanelDetails(Panel panel) => _buildTypeDetails([
        _buildSection('Display', [
          _buildDetailRow(
              'Tamanho da Tela', panel.screenSize, Icons.aspect_ratio),
          _buildDetailRow(
              'Resolução', panel.resolution, Icons.photo_size_select_large),
          if (panel.brightness != null)
            _buildDetailRow(
                'Brilho', '${panel.brightness}%', Icons.brightness_6),
          if (panel.volume != null)
            _buildDetailRow('Volume', '${panel.volume}%', Icons.volume_up),
        ]),
        _buildSection('Rede', [
          _buildDetailRow('Endereço IP', panel.ipAddress, Icons.network_check,
              copyable: true),
          _buildDetailRow('MAC Address', panel.macAddress, Icons.router,
              copyable: true),
        ]),
        _buildSection('Sistema', [
          _buildDetailRow(
              'Firmware', panel.firmwareVersion, Icons.system_update),
          _buildDetailRow(
              'Entrada HDMI', panel.hdmiInput ?? 'N/D', Icons.hd),
        ]),
      ]);

  Widget _buildPrinterDetails(Printer printer) => _buildTypeDetails([
        _buildSection('Conectividade', [
          _buildDetailRow('Tipo de Conexão', printer.connectionType,
              Icons.settings_input_hdmi),
          if (printer.ipAddress != null)
            _buildDetailRow(
                'Endereço IP', printer.ipAddress!, Icons.network_check,
                copyable: true),
          if (printer.hostComputerName != null)
            _buildDetailRow('Computador Host', printer.hostComputerName!,
                Icons.computer),
        ]),
        _buildSection('Status', [
          _buildDetailRow('Status', printer.printerStatus, Icons.print),
          if (printer.errorMessage != null)
            _buildDetailRow('Erro', printer.errorMessage!, Icons.error,
                copyable: true),
        ]),
        _buildSection('Contadores', [
          if (printer.totalPageCount != null)
            _buildDetailRow('Total de Páginas',
                printer.totalPageCount.toString(), Icons.description),
          if (printer.colorPageCount != null)
            _buildDetailRow('Páginas Coloridas',
                printer.colorPageCount.toString(), Icons.color_lens),
          if (printer.blackWhitePageCount != null)
            _buildDetailRow('Páginas P&B',
                printer.blackWhitePageCount.toString(), Icons.filter_b_and_w),
        ]),
        _buildSection('Capacidades', [
          _buildDetailRow('Impressão Duplex',
              printer.isDuplex == true ? 'Sim' : 'Não', Icons.compare_arrows),
          _buildDetailRow('Impressão Colorida',
              printer.isColor == true ? 'Sim' : 'Não', Icons.palette),
        ]),
      ]);
